...
//-----------------------------------------------------------
	...
	puts(" Connected \n");
	//send(sfd1," Score sent of apti",255,0);
	//-----------------------------------------------------
    int sockfd, rec_sock[3];
    ...
    int i;
    for(i=0;i<1;i++)
    rec_sock[i] = accept(sockfd, NULL, NULL);
...
//-----------------------------------------------------------	    
	...
	printf("Connected \n");
	send(nsfd,"Connected to apt server",18,0);
	send(sfd1," Score sent of apti",255,0);

  
    //-----------------------------------------------------------

	int n = send_fd(rec_sock[0], nsfd);
